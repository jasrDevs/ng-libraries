    ...
    if (isPlatformBrowser(this.platformId)) {
      ...
      this.zone.runOutsideAngular(() => {
        if (this.directiveRef) {
          ...
          merge(
            fromEvent(element, 'ps-scroll-x').pipe(map(() => 'x')),
            fromEvent(element, 'ps-scroll-y').pipe(map(() => 'y')),
            fromEvent(element, 'ps-x-reach-end').pipe(map(() => 'right')),
            fromEvent(element, 'ps-y-reach-end').pipe(map(() => 'bottom')),
            fromEvent(element, 'ps-x-reach-start').pipe(map(() => 'left')),
            fromEvent(element, 'ps-y-reach-start').pipe(map(() => 'top'))
          )
            .pipe(takeUntil(this.ngDestroy))
            .subscribe((state: string) => {
  ...
  private checkPropagation(event: any, deltaX: number, deltaY: number): void {
    ...
    if (
      ...
      (this.usePropagationX &&
        (!this.allowPropagationX || this.scrollDirectionX !== scrollDirectionX)) ||
